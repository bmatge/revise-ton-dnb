"""Routes des fiches de révision (cartes mentales) maths.

Deux URL :

    GET /              → liste des 6 fiches
...
from app.mathematiques.fiches.data import get_fiche, list_fiches

logger = logging.getLogger(__name__)

router = APIRouter(tags=["mathematiques-fiches"])

_HERE = Path(__file__).resolve().parent
_APP_DIR = _HERE.parent.parent
_CORE_TEMPLATES = _APP_DIR / "core" / "templates"
_MATH_TEMPLATES = _HERE.parent / "templates"
_FICHES_TEMPLATES = _HERE / "templates"

templates = Jinja2Templates(
    directory=[str(_FICHES_TEMPLATES), str(_MATH_TEMPLATES), str(_CORE_TEMPLATES)]
)
