"""Routes des fiches de révision (cartes mentales) français.
...
from app.francais.fiches.data import get_fiche, list_fiches

logger = logging.getLogger(__name__)

router = APIRouter(tags=["francais-fiches"])

_HERE = Path(__file__).resolve().parent
_APP_DIR = _HERE.parent.parent
_CORE_TEMPLATES = _APP_DIR / "core" / "templates"
_FR_TEMPLATES = _HERE.parent / "templates"
_FICHES_TEMPLATES = _HERE / "templates"

templates = Jinja2Templates(
    directory=[str(_FICHES_TEMPLATES), str(_FR_TEMPLATES), str(_CORE_TEMPLATES)]
)
